...
- Proves: an actual LLM sits behind the same typed input/output contract
  ...
  contract. The deterministic scenarios remain the evidence for the
...
- At runtime, a structured rejection is the boundary working, not a
  ...
  success path, `typed --real` must produce a typed result; if it rejects
  on stage, treat the live-model demo as failed and continue with the
  deterministic scenarios (see [DEMO-SCRIPT.md](DEMO-SCRIPT.md)).
